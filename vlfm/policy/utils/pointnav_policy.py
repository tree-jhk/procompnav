...
def _torch_load_allow_config(path: str, **kwargs: Any) -> Any:
    """
    Helper that loads checkpoints with weights_only disabled to support pickled OmegaConf
    objects produced by older checkpoints while remaining compatible with versions of
    torch that do not expose the argument.
    """
...
class WrappedPointNavResNetPolicy:
    ...
    def act(
        ...
        self.pointnav_prev_actions = pointnav_action.detach().clone()
        # self.pointnav_test_recurrent_hidden_states = rnn_hidden_states
        self.pointnav_test_recurrent_hidden_states = rnn_hidden_states.detach()
...
def load_pointnav_policy(file_path: str) -> PointNavResNetTensorOutputPolicy:
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5":
            ...
            state_dict = _torch_load_allow_config(file_path + ".state_dict", map_location="cpu")
        else:
            ckpt_dict = _torch_load_allow_config(file_path, map_location="cpu")
