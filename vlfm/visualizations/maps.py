...
cv2 = try_cv2_import()
from habitat.utils.visualizations.maps import AGENT_SPRITE
...
# our new colours
MAP_DISTRACTOR_COLOR = 10

TOP_DOWN_MAP_COLORS = np.full((256, 3), 150, dtype=np.uint8)
TOP_DOWN_MAP_COLORS[11:] = cv2.applyColorMap(
    np.arange(245, dtype=np.uint8), cv2.COLORMAP_JET
).squeeze(1)[:, ::-1]
...
TOP_DOWN_MAP_COLORS[MAP_DISTRACTOR_COLOR] =  [255, 204, 153] # yellow
...
def colorize_draw_agent_and_fit_to_height(
    ...
    rotate_circles = topdown_map_info.get("rotate_circles")
    if rotate_circles:
        for circle in rotate_circles:
            center = circle.get("center")
            radius = int(circle.get("radius", 1))
            if center is None or radius <= 0:
                continue
            draw_center = (int(center[1]), int(center[0]))
            cv2.circle(top_down_map, draw_center, radius, color=(0, 255, 0), thickness=2)
